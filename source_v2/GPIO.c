...
void GPIO_data_direction_port(gpio_port_name_t portName ,uint32_t direction);		 // f(x) #4
void GPIO_data_direction_pin (gpio_port_name_t portName, uint8_t state, uint8_t pin);// f(x) #5
void GPIO_write_port(gpio_port_name_t portName, uint32_t data);// f(x) #6
uint32_t GPIO_read_port(gpio_port_name_t portName);			   //(f(x) #7
void GPIO_set_pin(gpio_port_name_t portName, uint8_t pin);	   // f(x) #8
uint8_t GPIO_read_pin(gpio_port_name_t portName, uint8_t pin); // f(x) #9
void GPIO_clear_pin (gpio_port_name_t portName, uint8_t pin);  // f(x) #10
