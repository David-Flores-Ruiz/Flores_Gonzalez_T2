...
/*! Mask for read or set the especific pin		  */
#define mask_shiftPin 0x00000001

uint8_t GPIO_clock_gating(gpio_port_name_t port_name)	// f(x) #2		ok jlpe!
{
...
void GPIO_data_direction_port(gpio_port_name_t port_name, gpio_port_direction_t direction)		   // f(x) #4	ok!
{
	switch (port_name) {
	case GPIO_A:/** GPIO A is selected*/
...
void GPIO_data_direction_pin (gpio_port_name_t port_name, gpio_port_direction_t state, uint8_t pin)// f(x) #5	ok!
{
	switch (port_name) {
	case GPIO_A:/** GPIO A is selected*/
...
uint32_t GPIO_read_port(gpio_port_name_t port_name)			   //(f(x) #7	ok!
{
	switch (port_name) {
	case GPIO_A:/** GPIO A is selected*/
...
void GPIO_set_pin(gpio_port_name_t port_name, uint8_t pin)	   // f(x) #8	ok!
{
	switch (port_name) {
	case GPIO_A:
...
uint8_t GPIO_read_pin(gpio_port_name_t port_name, uint8_t pin)  // f(x) #9	ok!
{
	switch (port_name) {
		case GPIO_A:/** GPIO A is selected*/
		...
		/**Successful configuration*/
		return (pin_readValue & (mask_shiftPin << pin));
}// end fuction
void GPIO_clear_pin(gpio_port_name_t port_name, uint8_t pin)   // f(x) #10	ok!
{
	switch (port_name) {
	case GPIO_A:
